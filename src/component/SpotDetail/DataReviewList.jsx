...
const DataReviewList = ({ dataId, onReviewUpdate }) => {
 ...
 const handleSubmitReview = async (e) => {
   ...
   try {
     ...
     if (result.code === 200) {
       ...
       setNewReview({ rating: 0, content: '' }); // 별점도 0으로 초기화

       // 리뷰 목록 새로고침
       fetchReviews(0);

       // 상위 컴포넌트에 리뷰 업데이트 알림 (리뷰 통계 API 호출해서 전달)
       if (onReviewUpdate) {
         fetchReviewStatsAndNotify();
       }
 ...
 // 리뷰 통계 조회 후 상위 컴포넌트에 알림
 const fetchReviewStatsAndNotify = async () => {
   try {
     const response = await fetch(`/api/reviews/stats/${dataId}`);
     if (response.ok) {
       const stats = await response.json();
       onReviewUpdate(stats.totalCount, stats.averageRating);
     }
   } catch (err) {
     console.error('리뷰 통계 조회 오류:', err);
   }
 };
 ...
 // 컴포넌트 마운트 시 리뷰 조회 - dataId가 있을 때만
 useEffect(() => {
   if (dataId) {
     fetchReviews(0);
   }
       ...
       <div style={{
         ...
         <div style={{
           ...
           <form onSubmit={handleSubmitReview}>
             ...
             <button
               type="submit"
               disabled={isSubmitting || !newReview.content.trim() || newReview.rating === 0}
               style={{
                 display: 'flex',
                 alignItems: 'center',
                 justifyContent: 'center',
                 ...
                 backgroundColor: (isSubmitting || !newReview.content.trim() || newReview.rating === 0) ? '#9ca3af' : '#3b82f6',
                 color: 'white',
                 border: 'none',
                 borderRadius: '8px',
                 fontSize: 'clamp(12px, 2vw, 14px)',
                 fontWeight: '500',
                 cursor: (isSubmitting || !newReview.content.trim() || newReview.rating === 0) ? 'not-allowed' : 'pointer',
                 transition: 'all 0.2s',
                 width: '100%',
                 maxWidth: '200px'
               }}
